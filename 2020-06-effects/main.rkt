...
#;(begin
  #;(slides ([s:baseline hide])
    #:timeline (next) (tl:show s:baseline)
    (vc-append 30 (countdown-mtl)
               ((s:baseline) (c:plain (arrowhead 30 (turns -1/4))))
               ((s:baseline) (countdown-baseline))))

  (section "GHC")

  (slides () @titlet{The GHC Compilation Model})

  (slides ([s:stage 1])
    #:timeline (tl:sequence s:stage (in-range 1 4))
    (paras #:align 'center #:stage (s:stage)
           (inset (with-size 50 @elem{GHC is an unusual compiler.})
                  0 0 0 30)
           @elem{It compiles Haskell, a very high-level language…}
           @elem{…using a highly@it{traditional} compilation model.}))

  (slides ([s:stage 1])
    #:timeline (tl:sequence s:stage (in-range 1 4))
    #:with current-para-align 'center
    (paras #:spacing '(lines 0.75) #:stage (s:stage)
           (~> (with-size 50 @elem{@circled[1]  GHC is an ahead-of-time (AOT) compiler.})
               (inset 0 0 0 30))
           @elem{This is the compilation model used by C, C++, and Rust.}
           (paras #:spacing 2
                  @elem{Many compilers for high-level langs are just-in-time (JIT):}
                  @elem{Java, C#/.NET, JavaScript, PyPy, etc.})))

  (slides ([s:pros hide] [s:pipeline hide] [s:pipeline-opacity 1] [s:pipeline-strikethrough hide]
                         [s:cons hide])
    #:with current-para-align 'center
    #:with current-para-width 600
    (vc-append 20 (paras @elem{AOT has major advantages:}
                         ((s:pros) @elem{more predictable, smaller runtime.}))
               ((s:pipeline) (scale p:pipeline 1.5))
               ((s:cons) @para{Big disadvantage: optimizer does not have access to runtime information.}))
    #:timeline
    (next) (tl:show s:pros s:pipeline) (s:pipeline-opacity 1/4)
    (tl:show s:pipeline-strikethrough s:cons)
    #:where
    (define p:arrow (~> (arrow-line #:line-length 150 #:arrow-size 15 #:line-width 3)
                        (adjust-pen #:cap 'butt)
                        (colorize shape-border-color)))
    (define p:ghc
      (cc-superimpose (filled-rounded-rectangle 100 50 5
                                                #:color shape-bg-color
                                                #:border-color shape-border-color
                                                #:border-width 2)
                      (~> (with-font "Concourse C4" @t{ghc})
                          (colorize text-secondary-color)
                          (inset 0 0 0 7))))
    (define p:pipeline
      (hc-append 15
                 (~> (hc-append 15 (p:file @tt{.hs})
                                (cc-superimpose p:arrow (inset p:ghc 0 0 7 0)))
                     (cellophane (s:pipeline-opacity))
                     (cc-superimpose _ (~> (hline 200 3)
                                           (adjust-pen #:color text-tertiary-color
                                                       #:width 3 #:cap 'butt)
                                           (rotate (turns 0.03))
                                           (inset -30 0 0 0)
                                           ((s:pipeline-strikethrough)))))
                 (p:file @tt{bin}))))

  (slides ([s:branch hide] [s:prob/95 hide] [s:prob/5 hide]
                           [s:higher hide] [s:call hide])
    (vc-append 100 ((s:branch) p:branch) (vc-append 50 ((s:higher) p:higher) ((s:call) p:call)))
    #:timeline
    (tl:show s:branch s:prob/95 s:prob/5 s:higher s:call)
    (c:highlights '[add1]) (next)
    #:where
    (define (probability s)
      (~> (hc-append 15 (~> (arrow-line #:line-length 35)
                            (rotate (turns 1/2))
                            c:plain)
                     (with-size 32 @elem{@|s|%}))
          one-line))
    (define p:branch
      @haskell{
        case x of
          Foo -> handleFoo @((s:prob/95) (probability "95"))
          Bar -> handleBar @((s:prob/5) (probability "5"))})

    (define p:higher
      @haskell{
        module M (twiddle) where
          twiddle :: (Int -> Int) -> T -> T
          twiddle f x = ...})
    (define p:call
      @haskell{twiddle @t:add1{(+ 1)} x}))

  (define (ext p) (cc-superimpose (ghost @tt{.hs}) p))

  (define (down-arrow len)
      (~> (arrow-line #:line-length len)
          (rotate (turns -1/4))
          (colorize shape-border-color)))

  (slides ([s:files hide] [s:objects hide] [s:bin? #f]
                          [s:hs-changed? #f] [s:o-changed? #f] [s:bin-changed? #f])
    #:timeline (next) (tl:show s:files s:objects)
    (tl:flags s:bin? s:hs-changed? s:o-changed? s:bin-changed?)
    (vc-append 30 @elem{@circled[2]  GHC uses the traditional linking model.}
               (~> (vc-append 10 (hc-append 30
                                            (hs/o-files)
                                            (hs/o-files)
                                            (hs/o-files #:hs-changed? (s:hs-changed?)
                                                        #:o-changed? (s:o-changed?))
                                            (hs/o-files))
                              (inset p:bin-arrow 0 10 0 0)
                              (show (p:file/changed @tt{bin} (s:bin-changed?)) (s:bin?)))
                   connect-os-to-bin
                   ((s:files))))
    #:where
    (define (p:file/changed p changed?)
      (p:file p #:color (if changed? red-brush-color shape-bg-color)
              #:border-color (if changed? red-pen-color shape-border-color)))
    (define (hs/o-files #:hs-changed? [hs-changed? #f]
                        #:o-changed? [o-changed? #f])
      (vc-append 10 (p:file/changed @tt{.hs} hs-changed?)
                 ((s:objects) (down-arrow 15))
                 ((s:objects) (p:file/changed (ext @tt{.o}) o-changed?))
                 (t:.o (~> (vline 2 10)
                           (adjust-pen #:color shape-border-color
                                       #:width 2)
                           (show (s:bin?))))))
    (define p:bin-arrow (show (down-arrow 10) (s:bin?)))
    (define (connect-os-to-bin p)
      (if (s:bin?)
          (for/fold ([p p])
                    ([o-path (in-list (find-tag* p '.o))])
            (pin-line p o-path cb-find p:bin-arrow ct-find
                      #:line-width 2
                      #:color shape-border-color))
          p)))

  (slides ([s:info hide] [s:results hide] [s:bullet 0])
    #:timeline (next) (tl:show s:info s:results) (tl:sequence s:bullet (in-range 1 5))
    #:with current-para-fill? #f
    #:with current-para-width 600
    (vc-append 40 (with-size 50 @elem{However, Haskell does@it{not} use header files.})
               ((s:info)
                (hc-append 50 (scale files 1.5)
                           (paras #:stage (s:bullet)
                                  @elem{@tt{.hi} — “Haskell interface”}
                                  @item{type/class/instance declarations}
                                  @item{types of exported bindings}
                                  @item{source code of small functions (aka “unfoldings”)}))))
    #:where
    (define files (vc-append 10 (p:file @tt{.hs})
                             ((s:results) (hc-append 25 (rotate (down-arrow 20) (turns -0.1))
                                                     (rotate (down-arrow 20) (turns 0.1))))
                             ((s:results) (hc-append 45 (p:file (ext @tt{.o})) (p:file @tt{.hi}))))))
  ...
  (slides ([s:java 0] [s:haskell 0])
    #:timeline (tl:sequence s:java 3) (tl:sequence s:haskell (in-range 1 4))
    #:with get-current-code-font-size (const 35)
    (vc-append 50 (with-size 50 @elem{@circled[3]  GHC performs full type erasure.})
               (vl-append 50
                          (pict-when (> (s:java) 0) p:java)
                          (pict-when (> (s:haskell) 0) p:haskell)))
    #:where
  ...
  (slides ([s:bullet 0])
    ...
    #:with current-para-fill? #f
    #:with current-para-width 600
    (ol #:make-num circled #:sep (em 1/2) #:spacing '(lines 0.6) #:stage (s:bullet)
        @para{GHC is an AOT compiler.}
        @para{GHC uses the traditional linking model for separate compilation.}
        @para{GHC performs full type erasure.}))

  (slides () @elem{How do you compile typeclasses?})

  (slides ([s:bullet 0])
    #:timeline (tl:sequence s:bullet (in-range 1 5))
    (paras #:align 'center #:stage (s:bullet)
           (with-size 45 @elem{Non-Solution 1: Type Dispatch})
           (inset p:show 0 20)
           @elem{Directly violates rule@circled[3 #:inline? #t], type erasure.}
           @elem{Also violates rule@circled[2 #:inline? #t], separate compilation!})
    #:where
    (define p:show
      @haskell{
        show x = case typeOf x of
 ...
 exclaim x = show x ++ "!"})
  ...
  (slides ([s:sample 0] [s:bloaty hide] [s:violates hide])
    #:timeline (tl:sequence s:sample 4) (tl:show s:bloaty s:violates)
    (paras #:align 'center #:spacing '(lines 1)
           (with-size 45 @elem{Non-Solution 2: Monomorphization})
           (paras #:spacing '(lines 0.75) #:stage (s:sample)
                  (p:exclaim)
                  p:exclaim/Bool
                  p:exclaim/String)
           ((s:bloaty) @elem{Extremely code bloaty! @((s:violates) p:violates)}))
    #:where
    (define p:exclaim/Bool
      @haskell{
        exclaim_Bool :: Bool -> String
        exclaim_Bool x = show_Bool x ++ "!"})
    (define p:exclaim/String
      @haskell{
        exclaim_String :: String -> String
        exclaim_String x = show_String x ++ "!"})
    (define p:violates
      @elem{Violates rule@circled[2 #:inline? #t], separate compilation.}))
  ...
  (slides ([s:defn hide] [s:defn/ho hide] [s:call hide] [s:call/ho hide])
    #:timeline (c:highlights '[new-ty new-arg]) (next) (tl:show s:defn s:defn/ho s:call s:call/ho)
    (paras #:align 'center #:spacing '(lines 1)
           (with-size 45 @elem{Solution: Higher-Order Functions})
           (paras #:align 'center #:spacing '(lines 0.5)
                  ((s:defn) (p:exclaim))
                  ((s:defn/ho) step-arrow:down)
                  ((s:defn/ho) (p:exclaim/ho))
                  p:call))
    #:with current-code-line-sep 4
    #:where
    (define p:call
      (~> (hc-append 30 ((s:call) @haskell{exclaim True})
                     ((s:call/ho) step-arrow:right)
                     ((s:call/ho) @haskell{exclaim @t:new-arg{show_Bool} True}))
          (inset 0 30 0 0))))

  (slides ([s:stage 1])
    #:timeline (tl:sequence s:stage (in-range 1 3))
    (paras #:align 'center #:stage (s:stage)
           (with-size 50 @elem{This approach is quite elegant!})
           @elem{However, it has a hidden cost.}))
  ...
  (define (balloon w h
    ...
    (send path line-to s (- s))
    (send path line-to (* 2 s) 0)
    (send path line-to (- w r) 0)
    (send path arc (- w r) 0 r r (turns 1/4) 0 #f)
    (send path line-to w (- h r))
    (send path arc (- w r) (- h r) r r 0 (turns -1/4) #f)
    (send path line-to r h)
    (send path arc 0 (- h r) r r (turns 3/4) (turns 1/2) #f)
    (send path close)
    (~> (dc (λ (dc dx dy) (send dc draw-path path dx dy)) w h)
        (adjust-pen #:color bc #:width bw)
        (maybe-colorize c)))

  (define (wrap-balloon p #:padding [padding 25])
    (cc-superimpose (balloon (+ (pict-width p) (* padding 2))
                             (+ (pict-height p) (* padding 2)))
                    p))

  (define (pin-balloon #:show? [show? #t] base path find p)
    (pin-over base path (adjust-find find -25 25) (show (wrap-balloon p) show?)))

  (define title:known-unknown "Known vs. Unknown Calls")
  (slides ([s:stage 0] [s:defn? #f])
    #:title title:known-unknown
    (~> (paras #:align 'center #:spacing 20 #:stage (s:stage)
               @haskell{@p:fst ("hello", "world")}
               (vc-append 20 step-arrow:down @haskell{@t:lam{(\(x, _) -> x)} ("hello", "world")})
               (vc-append 20 step-arrow:down @haskell{"hello"}))
    ...
    #:timeline (next)
    (s:stage 1) (next) (s:defn? #t) (next!)
    (s:stage 2) (s:defn? #f) (c:highlights '[lam]) (next)
    (s:stage 3) (c:highlights '[]) (next)
  ...
  (slides ([s:stage 1] [s:defn? #f])
    #:timeline
    (next) (c:highlights '[f/use]) (next) (s:defn? #t) (next) (c:highlights '[f/use f/bind])
    (next) (s:stage 2) (next)
    ...
    (paras #:align 'center #:spacing 20 #:stage (s:stage)
           (~> defn:map
               (pin-balloon #:show? (s:defn?) f/use (adjust-find cbl-find 0 10) @elem{???})
               panorama)
           @elem{Unknown calls are@it{hard stops} for the optimizer.})
  ...
  (slides ([s:stage 1])
    #:timeline (c:highlights '[new-arg]) (tl:sequence s:stage (in-range 1 4))
    (paras #:align 'center #:spacing 50 #:stage (s:stage)
           @elem{Typeclass overloading creates unknown calls!}
           (paras #:align 'center #:spacing 20
                  (p:exclaim)
                  step-arrow:down
                  (p:exclaim/ho))
           (with-size 50 @elem{Overloading is@bt{not} free.}))))
